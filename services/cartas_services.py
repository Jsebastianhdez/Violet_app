...
def crear_carta(datos):
    ...
    if not dolls_activas:
        #Si no hay dolls, la carta sigue en espera
    ...
    if not doll:
        #Si todas las dolls están ocupadas, la carta sigue en espera
        datos["estado"] = "en espera"
        datos["doll_id"] = None
        return guardar_carta(datos)

    #Se asigna doll si esta disponible
